    ...
    const pairs = Object.entries(queryObj).map(([key, val]) => {
      if (val !== null && typeof val === "object" && !(val instanceof Date)) {
        ...
        return objectToQueryString(
          val,
          options,
          l_nesting_start + `${key}` + l_nesting_end,
          level + 1
        );
      } else {
        const numKey =
          options && options.numericKeys && options.numericKeys === true ? true : false;
        const emptyBrackets =
          options && options.emptyBrackets && options.emptyBrackets === true ? true : false;
